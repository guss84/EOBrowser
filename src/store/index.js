...
import get from 'dlv';
import { getMultipliedLayers, b64EncodeUnicode, updatePinsOnServer } from '../utils/utils';
...
function changePinOrder(oldIndex, newIndex, readOnly) {
  ...
  if (readOnly) {
    ...
    updatePinsOnServer(newPins).catch(e => {
      console.error(e);
    });
    return {
      userPins: newPins,
...
function addPins(items) {
  ...
  updatePinsOnServer(userPins).catch(e => {
    console.error(e);
  });
  return {
    userPins,
...
function removePin(index) {
  ...
  updatePinsOnServer(result).catch(e => {
    console.error(e);
  });
  return {
    userPins: result,
